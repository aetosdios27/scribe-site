...
/* vendored from interior.dev (ddoemonn/interior, MIT) — behaviour kept,
   styling reskinned to scribe tokens. */
...
export function TextReveal({
  ...
  const { ref, groups, step, started, reduced, delay: startDelay } =
    useTextReveal<HTMLSpanElement>(
      { text, by, stagger, maxDuration, startOnView, play, once, amount, delay },
    );

  return (
    <span ref={ref} className={`text-scribe-ink ${className}`}>
                  ...
                  transition={
                    reduced
                      ...
                      : {
                          duration: DURATION,
                          ease: EASE,
                          delay: started ? startDelay + unit.index * step : 0,
                        }
                  }
                >
                  {unit.text}
