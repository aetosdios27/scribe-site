...
export function PressDepth({
  ...
  return (
    <button
      ...
      className="group relative inline-flex select-none rounded-xs align-middle outline-none disabled:opacity-50"
      {...bind}
    >
      <span
        aria-hidden
        style={{ top: depth }}
        className="absolute inset-x-0 bottom-0 rounded-xs bg-scribe-rule-strong"
      />
      <motion.span
        initial={false}
        animate={{
        ...
        className={`relative inline-flex h-9 items-center justify-center gap-2 rounded-xs border border-scribe-rule-strong bg-scribe-paper-raised px-3.5 font-mono text-[13px] font-medium text-scribe-ink transition-colors group-focus-visible:outline-2 group-focus-visible:outline-scribe-cobalt ${className}`}
      >
        <motion.span
          aria-hidden
          initial={false}
          ...
          transition={reduced ? { duration: 0 } : PRESS}
          className="pointer-events-none absolute inset-0 rounded-xs shadow-[inset_0_1.5px_0_rgba(255,255,255,0.95),inset_0_-1px_0_rgba(10,10,10,0.06)]"
        />
        {children}
      </motion.span>
    </button>
