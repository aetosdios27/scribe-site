...
export function Accordion({
  ...
  return (
    <div
      className={`divide-y divide-scribe-rule overflow-hidden border border-scribe-rule-strong bg-scribe-paper-raised ${className}`}
    >
      {items.map((item) => (
        <AccordionRow
          key={item.id}
  ...
  return (
    <div>
      <div role="heading" aria-level={headingLevel}>
        <button
          {...header}
          className="flex w-full items-center gap-3 px-5 py-4 text-left outline-none transition-colors duration-150 hover:bg-scribe-paper focus-visible:bg-scribe-paper focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-scribe-cobalt"
        >
          <span
            className={`min-w-0 flex-1 truncate font-mono text-[13px] font-medium tracking-tight transition-colors duration-150 ${
              open ? "text-scribe-ink" : "text-scribe-muted"
            }`}
          >
            {item.title}
          </span>
          ...
          {item.meta ? (
            <span className="shrink-0 font-mono text-[11.5px] text-scribe-muted tabular-nums">
              {item.meta}
            </span>
          ) : null}
          ...
          <motion.svg
            ...
            fill="none"
            aria-hidden="true"
            className="shrink-0 text-scribe-ink"
            initial={false}
      ...
      >
        <div
          {...panel}
          ref={ref}
          className="border-t border-scribe-rule bg-scribe-paper shadow-[inset_0_1px_2px_rgba(10,10,10,0.04)]"
          style={{
        ...
        >
          <motion.div
            ...
            className="px-5 pt-3 pb-4 font-mono text-[13px] leading-relaxed text-scribe-muted"
          >
            {item.content}
          </motion.div>
        </div>
