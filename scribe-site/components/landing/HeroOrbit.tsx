...
/* fig. 1 - publish orbit. three tiers, best first:
   webgl2 halftone planet (GlobeCanvas) -> css-animated svg orbit ->
   static dithered png. whichever tier the browser can drive is shown. */

export function HeroOrbit() {
  return (
    <figure className="relative w-full max-w-[400px] lg:max-w-[519px]">
      ...
      <img
        ...
        fetchPriority="high"
        decoding="async"
        className="h-auto w-full [:has(~canvas[data-active=true])]:invisible"
      />

      <svg
        viewBox="0 0 600 640"
        aria-hidden="true"
        className="absolute inset-0 h-full w-full [:has(~canvas[data-active=true])]:hidden"
      >
